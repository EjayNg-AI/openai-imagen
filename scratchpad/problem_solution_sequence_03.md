...
Selected approach indices: 1, 3 (combination, refined with martingale/SA tools as suggested)

---

## 1. Reduction to a 1‑dimensional process

I start exactly as before, but I will be a bit more systematic because this reduction is the backbone of everything.
...
Every step you add exactly one blue ball, no matter which colour is drawn:

- If you draw red: you add \(+1\) red, \(+1\) blue.
- If you draw blue: you add \(+1\) blue and \(2k+1\) red for some \(k\).

Thus, deterministically,

\[
B_n = 1 + n\quad\text{for all }n.
\]

### 1.2. Red balls

Count red contributions up to time \(n\):

- There are \(n - K_n\) red draws; each adds \(1\) red.
- The \(k\)-th blue draw (\(k=1,\dots,K_n\)) adds \(2k+1\) red balls.
...
R_n
&= 1 + (n-K_n)\cdot 1 + \sum_{k=1}^{K_n} (2k+1)\\
&= 1 + n - K_n + \Bigl(2\sum_{k=1}^{K_n} k + \sum_{k=1}^{K_n} 1\Bigr)\\
...
The whole problem of asymptotics for \(G_n\) is equivalent to understanding the growth of the one–dimensional process \(K_n\).

---

## 2. Dynamics of the blue–draw count \(K_n\)

Let \(\mathcal F_n\) be the sigma–algebra generated by the first \(n\) draws.  

Let \(\xi_{n+1}\in\{0,1\}\) indicate whether the \((n+1)\)-th draw is blue:

\[
\xi_{n+1} = \mathbf{1}\{\text{blue at time }n+1\}.
\]
...
Given \(\mathcal F_n\), the probability the \((n+1)\)-th draw is blue is
...
\[
\boxed{
K_{n+1} = K_n + \xi_{n+1},\qquad
\mathbb E[\xi_{n+1}\mid\mathcal F_n] = p_n
= \frac{n+1}{K_n^2 + K_n + 2n + 2}.
}
\]
...
\[
\mathrm{Var}(\xi_{n+1}\mid\mathcal F_n) = p_n (1-p_n)\le p_n.
\]

---

## 3. First heuristics: exponent and candidate constant

### 3.1. Expected drift and exponent of \(K_n\)

Assume heuristically that for some \(\beta>0\), \(K_n\) typically behaves like \(K_n \approx C n^\beta\) for large \(n\).

For such \(n\), if \(\beta>1/2\), then \(K_n^2\gg n\), so in the denominator of \(p_n\) we may ignore the \(2n+2\) term and approximate
...
\[
\mathbb E K_n \approx \sum_{j=1}^n p_j
\approx \frac{1}{C^2}\sum_{j=1}^n j^{1-2\beta}
\asymp \frac{1}{C^2} n^{2-2\beta}.
\]

Self–consistency demands the exponent of \(n\) on both sides match:

\[
\beta = 2 - 2\beta \quad\Longrightarrow\quad \beta = \frac{2}{3}.
\]

So \(K_n\) should grow like \(n^{2/3}\).  

Then from \(G_n = K_n^2+K_n+2n+2\) we get:

- \(K_n^2 \sim \text{const} \cdot n^{4/3}\),
- the terms \(K_n\) and \(2n\) are lower–order (\(n^{2/3}\), \(n\)) compared to \(n^{4/3}\).

Hence we expect

\[
G_n \sim C^2 n^{4/3} \quad\text{a.s.}
\]

for some constant \(C>0\), and we conjecture \(\alpha=4/3\).

### 3.2. A smoother scale: \(Y_n := K_n^3/n^2\)
...
K(n)^3 \approx \frac{3}{2}n^2,
\qquad
K(n) \approx \left(\frac{3}{2}\right)^{1/3} n^{2/3}.
\]

Motivated by that, define
...
\[
Y_n \to \frac{3}{2} \quad\text{a.s.},
\]
then
\[
K_n \sim \left(\frac{3}{2}\right)^{1/3}n^{2/3},\quad
G_n = K_n^2+K_n+2n+2 \sim \left(\frac{3}{2}\right)^{2/3} n^{4/3},
\]
and the problem would be solved with
\[
\alpha = \frac{4}{3},\qquad c = \left(\frac{3}{2}\right)^{2/3}.
\]

The point of introducing \(Y_n\) is that its drift turns out to be nearly **linear** in \(Y_n\), an ideal setting for stochastic approximation.

---

## 4. Exact drift of \(Y_n\)

We now compute the one–step change of \(Y_n\) **exactly**.

Recall \(K_{n+1} = K_n + \xi_{n+1}\), where \(\xi_{n+1}\in\{0,1\}\). Then
...
= -\,\frac{2n+1}{n^2(n+1)^2}.
\]
Since \(K_n^3 = Y_n n^2\), the first term equals
\[
K_n^3\left(\frac{1}{(n+1)^2}-\frac{1}{n^2}\right)
= Y_n n^2\cdot\left(-\frac{2n+1}{n^2(n+1)^2}\right)
= -Y_n\,\frac{2n+1}{(n+1)^2}.
\]

Taking conditional expectation given \(\mathcal F_n\), and recalling \(\mathbb E[\xi_{n+1}\mid\mathcal F_n]=p_n\), we get
...
Now write \(D_n := K_n^2+K_n+2n+2\), so \(p_n=(n+1)/D_n\). Then
...
since
\[
3D_n = 3(K_n^2+K_n+2n+2) = 3K_n^2+3K_n+6n+6.
\]

So the second term becomes
\[
\frac{3K_n^2+3K_n+1}{(n+1)D_n}
= \frac{3D_n - (6n+5)}{(n+1)D_n}
= \frac{3}{n+1} - \frac{6n+5}{(n+1)D_n}.
\]
...
This is our exact drift formula.

We can rewrite slightly more suggestively. Note
\[
\frac{2n+1}{(n+1)^2}
= \frac{2}{n+1} - \frac{1}{(n+1)^2},
\]
so
\[
-Y_n\,\frac{2n+1}{(n+1)^2}
= -\frac{2Y_n}{n+1} + \frac{Y_n}{(n+1)^2}.
\]
Thus
...
The leading term is \(\frac{3-2Y_n}{n+1}\), which is exactly the drift of the ODE \(\dot y = 3-2y\) in **logarithmic time**. The last two terms are error terms that we must control.

---

## 5. Stochastic approximation heuristic for \(Y_n\)

If the extra terms were negligible, we would have
\[
\mathbb E[Y_{n+1}-Y_n\mid\mathcal F_n]
\approx \frac{3-2Y_n}{n+1},
\]
which is a classical **Robbins–Monro** / stochastic approximation recursion with step size \(\sim 1/n\) and drift
\[
F(y)=3-2y.
\]

The ODE \(\dot y = F(y) = 3-2y\) has a unique globally attracting equilibrium
\[
y_* = \frac{3}{2}.
\]

Standard SA/ODE theory (Benaïm, Borkar, Robbins–Siegmund) then suggests that \(Y_n\) should converge almost surely to \(y_*\). If we could rigorously prove that the extra terms in (4.4) form a summable perturbation and that the noise is square–integrable with \(\sum (1/n)^2\mathrm{Var}(\text{noise}_n)<\infty\), we would be done.

The remaining work is precisely to:

1. control the “error” terms in (4.4);
2. control the martingale (noise) part of \(Y_n\);
3. show that \(Y_n\) stays bounded and then apply a standard SA convergence theorem (e.g. Robbins–Siegmund for \(V_n=(Y_n-3/2)^2\)).

In this attempt, I go significantly further than before in setting up these steps, but I do not close all details.

---

## 6. Martingale decomposition for \(K_n\)

Before treating \(Y_n\), it’s convenient to write a martingale decomposition for \(K_n\).
...
Then \(M_n\) is a martingale with \(M_0=0\), and
\[
\boxed{
K_n = A_n + M_n.
}
...
This inequality \(\langle M\rangle_n \le A_n\) will be important when we apply martingale concentration inequalities (Freedman).

---

## 7. Coarse growth bounds for \(K_n\) (bootstrap stage – **partial**)

To control the error terms in (4.4) and the martingale noise in \(Y_n\), we need at least coarse information on the scale of \(K_n\). The evaluator correctly pointed out this was missing in my first attempt.

Here I outline how to get **two–sided polynomial bounds** on \(K_n\) using drift estimates and martingale inequalities. I will not carry out every technical detail (hence this remains a gap), but I make the structure explicit.

### 7.1. Aim

We want to show that, almost surely, there exist exponents
\[
\frac{1}{2} < \gamma_1 < \frac{2}{3} < \gamma_2 < 1
\]
and random \(N\) such that for all \(n\ge N\),

\[
n^{\gamma_1} \le K_n \le n^{\gamma_2}.
\]

This implies \(K_n^2\) dominates \(n^{1+\delta}\) for some \(\delta>0\) and is sublinear in \(n^2\). These bounds are enough to control the troublesome term \(\frac{6n+5}{(n+1)D_n}\) and the variance of the noise in \(Y_n\).

### 7.2. Upper bound on growth exponent of \(K_n\)

Fix \(\beta>2/3\). Define the event
\[
E_\beta := \{K_n \ge n^\beta \text{ infinitely often}\}.
\]

**Heuristic argument (to be made rigorous via Freedman + Borel–Cantelli):**

Assume on some large interval of times we have \(K_n \ge n^\beta\). Then for those \(n\),

\[
D_n = K_n^2 + K_n +2n+2 \ge K_n^2 \ge n^{2\beta}.
\]
So the drift
\[
p_n = \frac{n+1}{D_n} \le \frac{n+1}{n^{2\beta}} \lesssim n^{1-2\beta}.
\]

Hence on that interval the *expected* increment of \(K_n\) per step is at most \(C n^{1-2\beta}\). Summing over a block \([m,2m]\),
\[
\sum_{n=m}^{2m} p_n \lesssim \sum_{n=m}^{2m} n^{1-2\beta}
\asymp m^{2-2\beta}.
\]

If \(\beta>2/3\), then \(2-2\beta<2/3<\beta\). So expected growth over a dyadic block is of order \(m^{2-2\beta}\), much smaller than the threshold \(m^\beta\) we are supposing \(K_n\) reaches.

On the other hand, the martingale noise \(M_n\) on such a block has fluctuations of order at most \(\sqrt{A_{2m}-A_m}\), i.e., roughly the square–root of the sum of these small drifts; that is still much smaller than \(m^\beta\).

Using **Freedman’s inequality** (or Bernstein’s inequality for martingales with bounded increments), one can show:

> For each large block \([m,2m]\), the probability that \(K_n\) rises above \(m^\beta\) is at most \(\exp(-c m^{c'})\) for some constants \(c,c'>0\).

Summing over blocks and applying **Borel–Cantelli**, we deduce \(\mathbb P(E_\beta)=0\). Since this holds for each such \(\beta>2/3\), we get

\[
\limsup_{n\to\infty} \frac{\log K_n}{\log n} \le \frac{2}{3}\quad\text{a.s.}
\]

In slightly softer form: for some \(\gamma_2<1\) arbitrarily close to \(2/3\), we have \(K_n \le n^{\gamma_2}\) eventually almost surely.

### 7.3. Lower bound on growth exponent of \(K_n\)

Now fix \(\beta<2/3\). Define

\[
F_\beta := \{K_n \le n^\beta \text{ infinitely often}\}.
\]

Again, the idea is to show \(\mathbb P(F_\beta)=0\).

Suppose on some large interval, say \([m,2m]\), we have \(K_n \le n^\beta\). Then

\[
K_n^2 \le n^{2\beta}.
\]

If \(\beta>1/2\), then \(n^{2\beta}\gg n\) for large \(n\), so again \(K_n^2\) dominates the \(2n+2\) part in \(D_n\), i.e.,

\[
D_n = K_n^2 + K_n + 2n +2 \le c\,K_n^2 \le c n^{2\beta}
\]
for some constant \(c\). Thus

\[
p_n = \frac{n+1}{D_n} \gtrsim n^{1-2\beta}.
\]

In particular, on that interval the *expected* increment of \(K_n\) per step is at least \(c n^{1-2\beta}\). Summed over a block \([m,2m]\):

\[
\sum_{n=m}^{2m} p_n \gtrsim m^{2-2\beta}.
\]

Now if \(\beta<2/3\), then \(2-2\beta>2/3>\beta\). So over this block, the drift forces \(K_n\) to increase by an amount at least of order \(m^{2-2\beta}\gg m^\beta\).

Again, martingale fluctuations are smaller than the drift (on the scale of \(m^{1-\beta}\)), and by another application of Freedman’s inequality and Borel–Cantelli, one shows that

> with probability 1, for large \(m\) the event “\(K_n \le n^\beta\) on all \(n\in[m,2m]\)” does not occur.

Hence \(\mathbb P(F_\beta)=0\); since this holds for each \(\beta<2/3\), we obtain
...
> **Heuristic conclusion of the bootstrap stage:**  
> \[
> \lim_{n\to\infty} \frac{\log K_n}{\log n} = \frac{2}{3}\quad\text{a.s.},
> \]
> i.e., for every \(\varepsilon>0\), eventually almost surely
> \[
> n^{2/3-\varepsilon} \le K_n \le n^{2/3+\varepsilon}.
> \]

I emphasize that the above uses standard but nontrivial martingale concentration (Freedman) and Borel–Cantelli; in this write‑up I have outlined the structure but not every detail, so this remains **Gap 1**, albeit now much more concretely formulated.

For the rest of the analysis of \(Y_n\), I will assume this bootstrap result, i.e. that almost surely
\[
K_n = n^{2/3+o(1)},\qquad D_n = K_n^2 + \dots = n^{4/3+o(1)}.
\]

---

## 8. Refined analysis of \(Y_n\): SA stage – **partial but sharper**

### 8.1. Error decomposition

Recall the exact drift formula (4.4):
...
We now estimate \(\varepsilon_n\) using the bootstrap information.

- Since \(Y_n = K_n^3/n^2\) and \(K_n\asymp n^{2/3}\), we have \(Y_n=O(1)\), indeed \(Y_n\) remains of constant order (this is a heuristic that will later be self–confirmed, but I take it as part of the SA picture).
  Then
  \[
  \left|\frac{Y_n}{(n+1)^2}\right| \le \frac{C}{n^2},
  \]
  which is summable.

- For the last term, using \(D_n\asymp K_n^2\asymp n^{4/3}\), we get
  \[
  0 < \frac{6n+5}{(n+1)D_n}
  \lesssim \frac{n}{n^{4/3}} = n^{-1/3}.
  \]
  This decays only like \(n^{-1/3}\), **not summable**. However, this is the *raw bound*; when we convert to the SA form for a Lyapunov function \(V_n\), the effect on \(\varepsilon_n\) in that new variable will be milder.

At this point, it becomes more effective to work with
\[
V_n := (Y_n - 3/2)^2
\]
and apply the **Robbins–Siegmund almost supermartingale theorem** to \(V_n\).

### 8.2. Martingale noise in \(Y_n\)

From (4.1), the noise term in \(Y_{n+1}-Y_n\) is
...
It is a martingale difference: \(\mathbb E[N_{n+1}\mid\mathcal F_n]=0\), and
\[
|N_{n+1}|\le \frac{3K_n^2+3K_n+1}{(n+1)^2}.
\]
...
\mathbb E[N_{n+1}^2\mid\mathcal F_n]
\le \frac{(3K_n^2+3K_n+1)^2}{(n+1)^4} p_n
\lesssim \frac{K_n^4}{n^4} \cdot \frac{n}{K_n^2}
= \frac{K_n^2}{n^3}.
\]

Using \(K_n\asymp n^{2/3}\), the right–hand side is \(\asymp n^{4/3}/n^3 = n^{-5/3}\), which is summable:

\[
\sum_{n\ge1} n^{-5/3} < \infty.
\]

Thus, under the bootstrap, the martingale noise contribution in \(Y_n\) has square–summable conditional variance. By standard martingale convergence theorems, the series \(\sum N_{n+1}\) converges almost surely; in particular, its contribution to \(Y_n\) is \(O(1)\) and does not prevent convergence.

(Again, this step currently depends on the bootstrap scaling \(K_n\asymp n^{2/3}\); making it fully independent would require merging the bootstrap with the SA, which I have not done completely.)

### 8.3. Almost-supermartingale inequality for \(V_n=(Y_n-3/2)^2\)
...
We want to show that \((V_n)\) is an **almost supermartingale** in the sense of Robbins–Siegmund:

\[
\mathbb E[V_{n+1}\mid\mathcal F_n]
\le (1+\beta_n)V_n + \xi_n - \zeta_n,
\]
with
\[
\sum_n \beta_n <\infty,\quad
\sum_n \xi_n <\infty,
\]
and \(\zeta_n\) nonnegative. Then Robbins–Siegmund’s theorem implies that \(V_n\) converges almost surely and that \(\sum_n \zeta_n<\infty\); from the structure of \(\zeta_n\) we can deduce that the only possible limit is \(0\), i.e. \(Y_n\to 3/2\).
...
\begin{aligned}
V_{n+1} - V_n
&= \bigl(Y_{n+1}-\tfrac32\bigr)^2 - \bigl(Y_n-\tfrac32\bigr)^2\\
&= 2\bigl(Y_n-\tfrac32\bigr)(Y_{n+1}-Y_n) + (Y_{n+1}-Y_n)^2.
\end{aligned}
\]
...
= 2\bigl(Y_n-\tfrac32\bigr)\mathbb E[Y_{n+1}-Y_n\mid\mathcal F_n]
+ \mathbb E[(Y_{n+1}-Y_n)^2\mid\mathcal F_n].
\]

Split \(Y_{n+1}-Y_n\) into drift plus noise:
\[
Y_{n+1}-Y_n = \mathbb E[Y_{n+1}-Y_n\mid\mathcal F_n] + N_{n+1}.
\]
...
\mathbb E[(Y_{n+1}-Y_n)^2\mid\mathcal F_n]
= \mathbb E[N_{n+1}^2\mid\mathcal F_n]
+ \bigl(\mathbb E[Y_{n+1}-Y_n\mid\mathcal F_n]\bigr)^2.
\]

Now plug in the drift decomposition (leading term plus \(\varepsilon_n\)):
...
\begin{aligned}
2\bigl(Y_n-\tfrac32\bigr)\mathbb E[Y_{n+1}-Y_n\mid\mathcal F_n]
&= 2\bigl(Y_n-\tfrac32\bigr)\frac{3-2Y_n}{n+1}
+ 2\bigl(Y_n-\tfrac32\bigr)\varepsilon_n\\
&= -\frac{4}{n+1}\bigl(Y_n-\tfrac32\bigr)^2
+ 2\bigl(Y_n-\tfrac32\bigr)\varepsilon_n.
\end{aligned}
\]
...
where the \(O(1/(n+1)^2)\) comes from the square of the drift, which is \(O(1/(n+1)^2)\) if \(Y_n\) is bounded.

Now, under the bootstrap and the SA heuristics, **we expect** \(Y_n\) to remain in a compact interval \([c_1,c_2]\subset(0,\infty)\) eventually; in that regime:

- \(|Y_n-3/2|\le C\), so \(|2(Y_n-3/2)\varepsilon_n| \le C'|\varepsilon_n|\),
- \(\mathbb E[N_{n+1}^2\mid\mathcal F_n]\) is of order \(n^{-5/3}\), summable,
- the \(O(1/n^2)\) term is summable.

Moreover, with \(D_n\asymp n^{4/3}\),
\[
|\varepsilon_n|
= \left|\frac{Y_n}{(n+1)^2}
- \frac{6n+5}{(n+1)D_n}\right|
\le \frac{C}{(n+1)^2} + C' n^{-1/3}
\le C'' n^{-1/3}.
\]

So the error term in the drift of \(V_n\) is upper bounded by \(|2(Y_n-3/2)\varepsilon_n|\le C''' n^{-1/3}\).

Putting this together, we have an inequality of the form

\[
\mathbb E[V_{n+1}\mid\mathcal F_n]
\le V_n - \frac{4}{n+1}V_n
+ \underbrace{C''' n^{-1/3} + C'''' n^{-5/3}}_{=: \xi_n}.
\]

Crucially, \(\sum_n \xi_n<\infty\), since \(n^{-1/3}\) is not summable, but here we must be more delicate: the actual dependence of \(\varepsilon_n\) on \(Y_n\) and \(D_n\) may allow an improved bound of order \(n^{-1-\delta}\) for some \(\delta>0\); I have not fully derived this, which is part of the remaining gap.

Assuming we can refine the estimate of \(\varepsilon_n\) to have \(\sum |\varepsilon_n|<\infty\), then:

- \(\sum \mathbb E[N_{n+1}^2]<\infty\),
- \(\sum \xi_n<\infty\),

and we can then invoke the **Robbins–Siegmund theorem** to conclude:

1. \(V_n\) converges almost surely to a finite limit;
2. \(\sum \frac{4}{n+1}V_n<\infty\) a.s.

The second property implies that \(\liminf V_n=0\) a.s., and since the drift always pushes \(V_n\) downwards unless it is small, we deduce that the only possible limit is \(0\), i.e.,

\[
V_n \to 0\quad\text{a.s.}
\quad\Longrightarrow\quad
Y_n \to \frac{3}{2}\quad\text{a.s.}
\]

This would establish the desired a.s. convergence of \(Y_n\), and hence the asymptotics of \(K_n\) and \(G_n\).

In the current attempt, however, I have **not** fully justified:

- that \(Y_n\) remains bounded (though this is quite plausible from the sign of the drift for large \(Y_n\));
- that \(\sum |\varepsilon_n|<\infty\) almost surely (I only gave a crude bound using the bootstrap scaling, which is not yet fully proved).

So the application of Robbins–Siegmund remains at the level of a detailed plan, rather than a fully verified argument.

---

## 9. From \(Y_n\) to \(G_n\) (conditional on SA convergence)

Let me spell out the final step clearly, since it is easy once we have \(Y_n\to 3/2\).
...
Then \(K_n^3 \sim \frac{3}{2}n^2\), i.e.
\[
K_n \sim \left(\frac{3}{2}\right)^{1/3} n^{2/3}\quad\text{a.s.}
\]

So
\[
K_n^2 \sim \left(\frac{3}{2}\right)^{2/3} n^{4/3},\qquad
K_n = o(n^{4/3}),\qquad
n = o(n^{4/3}).
\]
...
- \(\dfrac{K_n^2}{n^{4/3}}\to \left(\frac{3}{2}\right)^{2/3}\) a.s.;
- \(\dfrac{K_n}{n^{4/3}} = \dfrac{K_n}{n^{2/3}}\cdot \dfrac{1}{n^{2/3}} \to \left(\frac{3}{2}\right)^{1/3}\cdot 0 = 0\) a.s.;
- \(2n/n^{4/3} = 2n^{-1/3}\to 0\);
- \(2/n^{4/3}\to 0\).
...
\[
\frac{G_n}{n^{4/3}} \longrightarrow \left(\frac{3}{2}\right)^{2/3}\quad\text{a.s.}
\]
...
## 10. Summary of achievements and remaining gaps

What is now solidly established:

1. **Reduction to \(K_n\):**  
   We have a completely rigorous reduction of the original urn process to the one–dimensional Markov chain \(K_n\), with the crucial identity
   \[
   G_n = K_n^2 + K_n + 2n + 2.
   \]

2. **Exact dynamics of \(K_n\):**  
   \(K_{n+1}=K_n+\xi_{n+1}\), where \(\mathbb E[\xi_{n+1}\mid\mathcal F_n]=p_n\) and
   \[
   p_n = \frac{n+1}{K_n^2+K_n+2n+2}.
   \]

3. **Exact dynamics of \(Y_n\):**  
   We derived the exact drift formula
   \[
   \mathbb E[Y_{n+1}-Y_n\mid\mathcal F_n]
   = \frac{3-2Y_n}{n+1}
   + \frac{Y_n}{(n+1)^2}
   - \frac{6n+5}{(n+1)(K_n^2+K_n+2n+2)}.
   \]

4. **Heuristic exponent and constant:**  
   Based on ODE heuristics and the SA form of the drift for \(Y_n\), we are led to
   \[
   \alpha = \frac{4}{3},\qquad
   c = \left(\frac{3}{2}\right)^{2/3}.
   \]

5. **Plan for a rigorous proof:**  
   A concrete two–stage plan is now in place:

   - **Bootstrap stage:** use martingale concentration (Freedman) plus drift bounds to prove that almost surely
     \[
     \lim_{n\to\infty}\frac{\log K_n}{\log n} = \frac{2}{3},
     \]
     i.e. \(K_n = n^{2/3+o(1)}\).
   - **SA stage:** with this scale control, show that \(Y_n\) satisfies an SA recursion with step size \(\sim 1/n\), drift \(3-2Y_n\) plus a summable perturbation, and square–summable noise; then apply Robbins–Siegmund (or Benaïm/Borkar SA theory) to deduce \(Y_n\to 3/2\) a.s., and finally conclude \(G_n/n^{4/3}\to (3/2)^{2/3}\) a.s.

What is **still missing** (logical gaps not completely closed):

- A fully rigorous execution of the bootstrap stage (two–sided power bounds for \(K_n\)) using Freedman and Borel–Cantelli.
- Precise bounds on the error term \(\varepsilon_n\) in the drift of \(Y_n\) that are **independent** of the heuristic scaling and provably summable.
- A complete verification of the hypotheses of a concrete stochastic approximation or almost–supermartingale theorem (Robbins–Siegmund) applied to \(V_n=(Y_n-3/2)^2\).

Accordingly, I keep the solution status as **Partial**.

---

## 11. Gaps

### Gap 1: Rigorous proof that \(K_n = n^{2/3+o(1)}\) a.s.

**Location:** Section 7 (bootstrap stage).  

**Description:**  

I outlined how to use drift bounds on \(p_n = (n+1)/(K_n^2+K_n+2n+2)\) together with Freedman’s inequality for martingales and Borel–Cantelli to show that for any \(\beta\neq 2/3\),

- the event \(\{K_n\ge n^\beta\ \text{i.o.}\}\) has probability 0 if \(\beta>2/3\),
- the event \(\{K_n\le n^\beta\ \text{i.o.}\}\) has probability 0 if \(\beta<2/3\).

But I did not fully define the stopping times or work out the explicit Freedman bounds and summations; the argument remains at the “blueprint” level.

**Reason for difficulty:**  

Carrying out the dyadic block decomposition, controlling \(\sum p_n\) on each block under the hypothesis \(K_n\ge n^\beta\) or \(\le n^\beta\), and then precisely quantifying deviations of the martingale part via Freedman/Bernstein, requires nontrivial but routine probability estimates. I have not filled in all technical details here.

---

### Gap 2: Summability of the drift error \(\varepsilon_n\) in \(Y_n\)
...
\[
\mathbb E[Y_{n+1}-Y_n\mid\mathcal F_n]
= \frac{3-2Y_n}{n+1} + \varepsilon_n,\quad
\varepsilon_n = \frac{Y_n}{(n+1)^2}
- \frac{6n+5}{(n+1)D_n},
\]
and argued informally that under the bootstrap scaling \(K_n\asymp n^{2/3}\), we have \(|\varepsilon_n| \le C n^{-1/3}\).

To fit Robbins–Siegmund’s framework cleanly, we would like \(\sum |\varepsilon_n|<\infty\) almost surely. I have not derived a bound of the form \(|\varepsilon_n|\le C n^{-1-\delta}\) with \(\delta>0\); instead I only have the weaker (and non-summable) estimate \(O(n^{-1/3})\).

**Reason for difficulty:**  

A more delicate analysis of \(\varepsilon_n\) must use not just the magnitude of \(K_n\) but also how close \(Y_n\) is to \(3/2\), since near equilibrium the effective effect of \(\varepsilon_n\) on the Lyapunov function \(V_n\) is smaller. Doing this correctly would require combining the bootstrap and the SA arguments in a more intertwined fashion, which I have not completed.

---

### Gap 3: Full verification of Robbins–Siegmund conditions for \(V_n\)

**Location:** Section 8.3.  

**Description:**  

I set up the Lyapunov function \(V_n=(Y_n-3/2)^2\) and its one–step conditional expectation. I argued heuristically that we should get
\[
\mathbb E[V_{n+1}\mid\mathcal F_n]
\le V_n - c\frac{V_n}{n} + \xi_n,
\]
with \(\sum \xi_n<\infty\), which would allow the application of the Robbins–Siegmund almost–supermartingale theorem to conclude \(V_n\to 0\). However:

- I did not explicitly derive such an inequality with concrete constants;
- I did not prove that \(Y_n\) is almost surely bounded (though drift negativity for large \(Y_n\) is suggestive of this);
- I did not state and check the exact Robbins–Siegmund theorem in full.

**Reason for difficulty:**  

To be fully rigorous, one must compute carefully the contributions of the error term \(\varepsilon_n\), the noise variance \(\mathbb E[N_{n+1}^2\mid\mathcal F_n]\), and the square of the drift, all in terms of \(V_n\) and \(n\). This is feasible but algebraically heavy. I have not completed this full check here.

---

## 12. Self‑evaluation

**Confidence score (0–100):** 65  

**Known or suspected issues:**

- The **main conceptual structure** (reduce to \(K_n\), use \(Y_n=K_n^3/n^2\), and apply stochastic approximation) is sound and fully consistent with standard techniques in the literature on non-linear urns and SA.
- However, the rigorous proof is still incomplete in two places:
  - almost-sure growth exponent \(2/3\) for \(K_n\) (bootstrap),
  - detailed SA verification for \(Y_n\)’s convergence, especially summability of the perturbation and boundedness.
- Some error estimates in Section 8.1 are still heuristic and depend on the very asymptotics they are supposed to help prove, although the structure for de‑circularizing them is now clearer.
- I have not fully written out the martingale inequalities (Freedman) and Borel–Cantelli steps, but only described how they should be applied.

**Coverage comment:**

- The reduction from the urn process to the process \(K_n\) and the formula \(G_n=K_n^2+K_n+2n+2\) is fully rigorous.
- The identification of the exponent \(\alpha=4/3\) and the candidate constant \(c=(3/2)^{2/3}\) is well supported by detailed drift and ODE analysis.
- The outline of a rigorous SA-based proof (using Freedman + Robbins–Siegmund) is now quite concrete, but key technical lemmas (polynomial bounds on \(K_n\), summability of certain error terms) remain to be fully proved.
- Therefore, the problem’s demand — **prove** there exist \(c,\alpha>0\) such that \(G_n/n^\alpha\to c\) almost surely — is not yet completely satisfied, though I believe the path to a complete proof is now well charted.
...
- **Location:** §7 (“Coarse growth bounds for \(K_n\) (bootstrap stage)”)  
  **Issue type:** Missing critical step / incomplete argument  
  **Description:**  
  The two-sided polynomial bounds \(K_n \approx n^{2/3}\) are still not rigorously proved. In §7.2–7.3 you sketch how, for each \(\beta\neq 2/3\), one should rule out events like \(\{K_n\ge n^\beta\ \text{i.o.}\}\) or \(\{K_n\le n^\beta\ \text{i.o.}\}\) using drift bounds and Freedman’s inequality, but you do not actually define the stopping times, state the martingale inequality with parameters, or carry out the Borel–Cantelli summations. This is not an optional refinement: without a proved lower bound like \(K_n^2 \gg n^{1+\delta}\), the later estimates on error terms and noise for \(Y_n\) are not justified.  
  **Suggested direction for fix:**  
  Make this into a precise lemma along the lines: “For any \(\beta>2/3\), \(\mathbb P(K_n\ge n^\beta\ \text{i.o.})=0\)” and similarly for \(\beta<2/3\). For example:  
  1. Work on dyadic blocks \(n\in[2^k,2^{k+1}]\). On the event that \(K_n \ge n^\beta\) throughout such a block, derive deterministic upper bounds on \(\sum p_n\) and on \(\langle M\rangle_n\).  
  2. Apply Freedman’s inequality (or a Bernstein-type inequality) to bound the probability that the martingale \(M_n\) deviates enough to allow \(K_n\) to reach/maintain \(n^\beta\) over the block.  
  3. Show these probabilities are summable over \(k\), and then invoke Borel–Cantelli.  
  Repeat for the lower bound with \(\beta<2/3\), now using a lower bound on \(p_n\).

- **Location:** §8.1 (“Error decomposition” for \(\varepsilon_n\)) and §8.3 (drift of \(V_n\))  
  **Issue type:** Logical gap / insufficient control of error term  
  **Description:**  
  You write  
  \[
  \mathbb E[Y_{n+1}-Y_n\mid\mathcal F_n]
   = \frac{3-2Y_n}{n+1} + \varepsilon_n,\quad 
  \varepsilon_n = \frac{Y_n}{(n+1)^2}-\frac{6n+5}{(n+1)D_n},
  \]
  and then use the heuristic \(K_n\asymp n^{2/3}\) to assert \(|\varepsilon_n|\lesssim n^{-1/3}\). This bound is not summable, so as it stands it is too weak for a clean Robbins–Siegmund application; and more importantly it still depends on the very scaling \(K_n\asymp n^{2/3}\) that you are trying to prove, i.e. the argument is circular. Moreover, when you move to the Lyapunov variable \(V_n=(Y_n-3/2)^2\), you only argue heuristically that the contribution from \(\varepsilon_n\) to the drift of \(V_n\) is harmless.  
  **Suggested direction for fix:**  
  You need a sharper estimate that uses not just crude size of \(K_n\) but also the fact that \(Y_n\) is *close to* \(3/2\) for large \(n\), so that \((Y_n-3/2)\varepsilon_n\) becomes of smaller order. One possible route:
  - First, prove coarse but nontrivial bounds on \(K_n\) (Gap 1).  
  - Then rewrite \(\varepsilon_n\) in terms of \(Y_n-3/2\) and \(K_n\) and show that, once \(Y_n\) enters a fixed compact interval around \(3/2\), \(|(Y_n-3/2)\varepsilon_n| \le C n^{-1-\delta}\) for some \(\delta>0\).  
  - Use this in the \(V_n\) recursion to get a genuinely summable error term.

- **Location:** §8.2 (“Martingale noise in \(Y_n\)”)  
  **Issue type:** Dependence on unproved growth rate / circular reasoning  
  **Description:**  
  The estimate
  \[
  \mathbb E[N_{n+1}^2\mid\mathcal F_n]
  \lesssim \frac{K_n^2}{n^3}
  \asymp \frac{n^{4/3}}{n^3} = n^{-5/3}
  \]
  is based on inserting the heuristic \(K_n\asymp n^{2/3}\). Without an a priori polynomial lower bound on \(K_n\), the best you can say is \(\mathbb E[N_{n+1}^2\mid\mathcal F_n] \lesssim K_n^2/n^3\), which is not directly comparable to \(n^{-5/3}\). So at this point you have not established that \(\sum_n \mathbb E[N_{n+1}^2\mid\mathcal F_n]<\infty\). Since square‑summability of the noise scale is one of the key SA assumptions, this is a serious gap.  
  **Suggested direction for fix:**  
  After proving that \(K_n\) lies between powers \(n^{\gamma_1}\) and \(n^{\gamma_2}\) with \(1/2<\gamma_1\le\gamma_2<1\) (Gap 1), substitute those *proven* bounds into  
  \[
  \mathbb E[N_{n+1}^2\mid\mathcal F_n]
  \le \frac{(3K_n^2+3K_n+1)^2}{(n+1)^4}p_n
  \]
  using the exact form of \(p_n\). You should be able to deduce for some \(\delta'>0\) that
  \[
  \mathbb E[N_{n+1}^2\mid\mathcal F_n] \le C n^{-1-\delta'},
  \]
  and hence \(\sum_n \mathbb E[N_{n+1}^2\mid\mathcal F_n]<\infty\) a.s. This breaks the circularity.

- **Location:** §8.3 (“Almost-supermartingale inequality for \(V_n\)”)  
  **Issue type:** Misuse / under-specification of theory  
  **Description:**  
  You outline how to write a recursion for \(V_n=(Y_n-3/2)^2\) and suggest it should satisfy an inequality of the form
  \[
  \mathbb E[V_{n+1}\mid\mathcal F_n]
   \le V_n - c\frac{V_n}{n} + \xi_n,
  \]
  sufficient for Robbins–Siegmund. But you do not actually produce such an inequality with explicit constants. Moreover, you repeatedly rely on phrases like “under the bootstrap and SA heuristics, we expect \(Y_n\) to remain in a compact interval” without a proof that \(Y_n\) is almost surely bounded; boundedness of the iterates is a central SA hypothesis. Without showing that for large \(Y_n\) the drift in \(Y_n\) (or in \(V_n\)) is significantly negative and that noise cannot counteract it, you cannot safely assume \(Y_n\) does not escape to infinity or decay to zero.  
  **Suggested direction for fix:**  
  - First, prove a *drift condition* of the form: there exists \(M<\infty\) such that whenever \(Y_n\ge M\), 
    \[
    \mathbb E[Y_{n+1}-Y_n\mid\mathcal F_n] \le -\frac{c}{n}
    \]
    for some \(c>0\). Combine with bounded noise to show \(Y_n\) is eventually confined to \([0,M]\).  
  - Next, explicitly compute \(\mathbb E[V_{n+1}-V_n\mid\mathcal F_n]\) in terms of \(V_n\), separate a negative term proportional to \(V_n/n\), and gather all remaining terms into a nonnegative \(\xi_n\) with \(\sum \xi_n<\infty\).  
  - Then quote Robbins–Siegmund with full statement and check each hypothesis.

- **Location:** Interaction of §§7–8 (bootstrap + SA)  
  **Issue type:** Structural / residual circularity  
  **Description:**  
  You have done a good job of *stating* the logical separation between the bootstrap stage (bounds on \(K_n\)) and the SA stage (analysis of \(Y_n\)). However, in the actual estimates, you still allow yourself to use heuristic asymptotics \(K_n\asymp n^{2/3}\) when analyzing \(\varepsilon_n\) and \(N_{n+1}\), rather than only previously proved bounds. This means parts of the SA analysis are still circular: they assume precisely the growth behaviour they seek to prove.  
  **Suggested direction for fix:**  
  Make the proof structure explicit and one-directional:
  1. Prove a lemma giving polynomial bounds on \(K_n\) *without* invoking \(Y_n\) at all.  
  2. In all subsequent estimates for \(\varepsilon_n\) and \(N_{n+1}\), use only those polynomial bounds (plus potentially bounds on \(Y_n\) that are already derived) and not the full \(K_n\sim n^{2/3}\).  
  3. Only at the very end, after having proved convergence of \(Y_n\), upgrade the coarse polynomial bounds on \(K_n\) to the sharp asymptotics.

---

## Minor issues  

- **Location:** §8.1 (“Error decomposition”), first bullet  
  **Nature:** Unjustified boundedness claim  
  **Description:**  
  You say “Since \(Y_n = K_n^3 / n^2\) and \(K_n\asymp n^{2/3}\), we have \(Y_n=O(1)\), indeed \(Y_n\) remains of constant order (this is a heuristic that will later be self–confirmed, but I take it as part of the SA picture).” Even as a heuristic, this is misleading: from the bootstrap you only know roughly \(K_n\in [n^{2/3-\varepsilon},n^{2/3+\varepsilon}]\), so a priori \(Y_n\) could grow like \(n^{3\varepsilon}\). The point of the SA analysis is to rule out such growth.  
  **Suggestion:**  
  Do not rely on “\(Y_n=O(1)\)” before it is proved. Work with explicit exponents \(\gamma_1,\gamma_2\) and write bounds like \(Y_n \le C n^{3\gamma_2-2}\) that you then show are incompatible with the drift for large \(Y_n\).

- **Location:** §8.1, comment on \(\frac{6n+5}{(n+1)D_n}\)  
  **Nature:** Overly optimistic phrasing  
  **Description:**  
  You mention that this term is \(O(n^{-1/3})\) and “not summable”, then say its effect on the Lyapunov function might be milder, but you do not make the dependence precise. This might confuse a reader into thinking that an \(O(n^{-1/3})\) error is harmless by itself, which it is not in a plain SA recursion with step size \(1/n\).  
  **Suggestion:**  
  Be explicit that an \(O(n^{-1/3})\) error in the *drift* is too large if it appears unweighted, and that you need to exploit the factor \((Y_n-3/2)\) in the Lyapunov equation to obtain a smaller effective error term.

- **Location:** General exposition  
  **Nature:** Mixing “heuristic” and “proved” statements  
  **Description:**  
  Throughout §§7–8 you correctly mark many arguments as heuristic or “blueprints”, but occasionally sentences read as if certain facts (e.g. \(K_n = n^{2/3+o(1)}\) a.s.) have been established.  
  **Suggestion:**  
  Wherever a conclusion depends on unproved steps, flag it explicitly as a conjectured consequence or as something that would follow once certain lemmas are proved. This will help you track exactly what remains to be done.

- **Location:** §8.2, end (“Thus, under the bootstrap, the martingale noise contribution … has square–summable conditional variance”)  
  **Nature:** Overstated certainty  
  **Description:**  
  You conclude square‑summable variance based on the heuristic estimate \(\asymp n^{-5/3}\). Given the circular dependence, this should be phrased conditional on established growth bounds for \(K_n\).  
  **Suggestion:**  
  Rephrase as: “If we can show that \(K_n\) grows at least like \(n^\gamma\) with \(\gamma>1/2\), then this bound becomes summable; later we will aim to prove such a growth bound.”

---

## Gap assessment  

The problem solver listed three main gaps; I agree with them and refine their classification:

- **Gap 1:** Rigorous proof that \(K_n = n^{2/3+o(1)}\) a.s.  
  **Assessment:** Fundamental.  
  This is not merely a technical refinement: essentially all later approximations for \(p_n\), \(D_n\), \(\varepsilon_n\), and \(N_{n+1}\) rely on knowing that \(K_n^2\) dominates \(n\) by a polynomial factor and that \(K_n\) doesn’t grow too fast. A fully rigorous law of large numbers for \(K_n\) (or at least sharp polynomial bounds) is the backbone of the whole analysis.

- **Gap 2:** Summability of the drift error \(\varepsilon_n\) in the SA recursion for \(Y_n\)  
  **Assessment:** Moderate.  
  Once Gap 1 is closed and you have decent bounds on \(K_n\), this becomes an exercise in careful algebra and using the fact that \(Y_n\) is not exploding to infinity. It is nontrivial but conceptually straightforward.

- **Gap 3:** Full verification of Robbins–Siegmund (or SA) conditions for \(V_n\)  
  **Assessment:** Moderate.  
  This requires choosing a precise convergence theorem, expanding the recursion for \(V_n\), and bounding terms. It’s standard SA machinery. The main work is systematic bookkeeping; no qualitatively new idea is needed.

So I would describe Gap 1 as addressing the *main difficulty of the problem*, while Gaps 2 and 3 are substantial but more technical steps that should fall into place once Gap 1 is handled carefully.
...
The original problem has a single goal: prove that there exist constants \(0<c,\alpha<\infty\) with
\[
...
- Completely and rigorously reduced the problem to the behaviour of \(K_n\) and derived the identity \(G_n=K_n^2+K_n+2n+2\).
- Derived an exact drift formula and a good potential function \(Y_n=K_n^3/n^2\).
- Identified the plausible scaling \(\alpha=4/3\) and constant \(c=(3/2)^{2/3}\).
- Outlined a concrete and standard two‑stage proof (bootstrap via Freedman; SA via Robbins–Siegmund).

However, the **actual almost-sure convergence** statement is still unproved: the core SA step “\(Y_n\to 3/2\) a.s.” is not established because both the bootstrap bounds for \(K_n\) and the SA error/noise conditions remain at a heuristic level. So the main theorem is not yet fully addressed.

---

## Summary  

This second attempt represents a clear and meaningful improvement over the first: several algebraic slips have been corrected (notably the drift of \(Y_n\) now has the right \(1/n\) scale), the exact drift formula for \(Y_n\) is cleanly derived, and the martingale structure for \(K_n\) is properly emphasized. The idea to use \(Y_n = K_n^3/n^2\) as the central potential remains very well chosen: the drift of \(Y_n\) is almost linear in \(Y_n\), matching the canonical Robbins–Monro setting with a single stable equilibrium at \(3/2\). Your decomposition of \(K_n\) as \(A_n + M_n\) (drift plus martingale) is standard and appropriate, and your plan to use Freedman’s inequality for the bootstrap followed by Robbins–Siegmund on \(V_n=(Y_n-3/2)^2\) is exactly the right toolkit.  

The main weaknesses are still on the side of rigor and closure. The bootstrap bounds for \(K_n\) are only sketched: while the strategy (working on dyadic blocks, bounding \(\sum p_n\), and controlling \(M_n\) via Freedman) is sound, the details are not supplied, and without them later approximations cannot be justified. In the SA stage, the error term \(\varepsilon_n\) in the drift of \(Y_n\) is only bounded by \(O(n^{-1/3})\) using unproved asymptotics for \(K_n\), which is not strong enough and introduces circularity. The variance bound for the martingale noise \(N_{n+1}\) similarly relies on \(K_n\asymp n^{2/3}\) before that fact has been established. Finally, the application of Robbins–Siegmund is only sketched; an explicit inequality for \(\mathbb E[V_{n+1}\mid\mathcal F_n]\) and verification of boundedness for \(Y_n\) are still missing.  

Overall, the conceptual path to a full solution is now quite clear and aligns well with established methods in the literature on non-linear urns and stochastic approximation. With careful attention to the bootstrap for \(K_n\) and a systematic SA verification for \(Y_n\), this approach should yield a complete proof. But in its present form, important pieces remain at the blueprint/heuristic level, so the solution must still be judged partial.

---

## Recommendations for next iteration  

1. **Complete the bootstrap for \(K_n\) rigorously.**  
   - Fix \(\beta>2/3\). On dyadic blocks \([2^k,2^{k+1}]\), assume \(K_n\ge n^\beta\) and derive deterministic bounds on \(p_n\) and hence on \(A_n=\sum p_n\) and \(\langle M\rangle_n\).  
   - Apply Freedman’s inequality to bound \(\mathbb P(\sup_{[2^k,2^{k+1}]}|M_n| \ge \text{threshold})\). Show this is summable in \(k\), and use Borel–Cantelli to conclude that \(K_n\ge n^\beta\) cannot hold infinitely often.  
   - Repeat for \(\beta<2/3\), now showing that \(K_n\le n^\beta\) infinitely often has probability 0. This will give the almost-sure exponent \(\log K_n/\log n\to 2/3\).

2. **Derive bounds on \(K_n\) that are strong enough to control \(D_n\) and noise without invoking exact asymptotics.**  
   - From the bootstrap, get explicit exponents \(1/2<\gamma_1<2/3<\gamma_2<1\) such that eventually \(n^{\gamma_1}\le K_n\le n^{\gamma_2}\).  
   - Convert these into inequalities like \(c_1 n^{2\gamma_1} \le D_n \le c_2 n^{2\gamma_2}\).  
   - Use these in all subsequent estimates for \(\varepsilon_n\) and \(\mathbb E[N_{n+1}^2\mid\mathcal F_n]\), instead of plugging in the “target” exponent \(2/3\).

3. **Re‑estimate the variance of the noise \(N_{n+1}\) using only the proven bounds for \(K_n\).**  
   - Start from  
     \[
     \mathbb E[N_{n+1}^2\mid\mathcal F_n]
     \le \frac{(3K_n^2+3K_n+1)^2}{(n+1)^4}p_n.
     \]  
   - Substitute bounds on \(K_n\) and \(p_n\) implied by your bootstrap stage to show that \(\mathbb E[N_{n+1}^2\mid\mathcal F_n] \le C n^{-1-\delta'}\) for some \(\delta'>0\).  
   - Conclude that \(\sum_n \mathbb E[N_{n+1}^2\mid\mathcal F_n]<\infty\), and hence the martingale \(\sum N_{n+1}\) converges a.s.

4. **Work directly with the Lyapunov function \(V_n=(Y_n-3/2)^2\) and derive an explicit almost‑supermartingale inequality.**  
   - Expand  
     \[
     V_{n+1} - V_n
      = 2(Y_n-3/2)(Y_{n+1}-Y_n) + (Y_{n+1}-Y_n)^2
     \]
     and plug in the drift/noise decomposition for \(Y_{n+1}-Y_n\).  
   - Separate a term of the form \(-cV_n/(n+1)\) (coming from the main drift \((3-2Y_n)/(n+1)\)) and gather all remainder terms into \(\xi_n\).  
   - Use your new bounds on \(\varepsilon_n\) and on the noise variance to show \(\sum \xi_n<\infty\).

5. **Establish boundedness of \(Y_n\) (or of \(V_n\)) explicitly.**  
   - Show that for sufficiently large \(Y_n\), the sign of the drift \(\mathbb E[Y_{n+1}-Y_n\mid\mathcal F_n]\) is negative and dominates noise, forcing \(Y_n\) back into a compact interval.  
   - Similarly, argue that \(Y_n\) cannot approach 0 indefinitely because then the drift becomes positive. This relies on the form of \(F(y)=3-2y\).

6. **State and apply a specific version of the Robbins–Siegmund theorem.**  
   - Quote the theorem with all hypotheses.  
   - Identify your choices of \(V_n\), \(\beta_n\), \(\xi_n\), and \(\zeta_n\) (with \(\zeta_n\approx cV_n/(n+1)\)).  
   - Check one by one: (i) nonnegativity of \(V_n\), (ii) \(\sum \beta_n<\infty\), (iii) \(\sum \xi_n<\infty\).  
   - Conclude that \(V_n\) converges a.s. and that \(\sum \zeta_n<\infty\), implying \(V_n\to 0\).

7. **After securing \(Y_n\to 3/2\) a.s., complete the final deduction for \(K_n\) and \(G_n\).**  
   - From \(K_n^3/n^2\to 3/2\) show \(K_n\sim (3/2)^{1/3}n^{2/3}\).  
   - Substitute into \(G_n=K_n^2+K_n+2n+2\) and show \(G_n/n^{4/3}\to (3/2)^{2/3}\).

8. **Keep the separation between heuristic and rigorous steps clear.**  
   - As you fill in the technical details, remove or relegate the heuristic uses of \(K_n\asymp n^{2/3}\) to an introductory “motivation” section.  
   - In the main proof, ensure that each estimate only uses previously proved bounds and that the logical dependency is strictly forward.
...
The remaining gaps, while technically nontrivial, fall squarely within the scope of standard stochastic approximation and martingale theory: Freedman/Bernstein inequalities, Robbins–Siegmund’s theorem, and the ODE method are all classical tools. Existing work on non-linear and unbalanced urn schemes demonstrates that similar arguments have been carried out successfully in closely related settings. No fundamentally new or research-level ideas appear to be required; the challenge is careful, detailed execution of the known methods.