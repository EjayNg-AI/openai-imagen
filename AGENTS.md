...
- Covers frontend interactions for image playground (FormData fetches, client-side checks, spinners/toasts, mask/out-paint UI) and responses playground (JSON POST, inline status, multi-turn prompt runner with copy buttons and assistant-only output).
